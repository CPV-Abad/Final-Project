...
def showHistory():
    ...
    programs_dropdown = ttk.Combobox(history_frame, state="readonly",
                                     textvariable=selected_app, width=25, font=("Arial", 10))
    ...
    def submitProgram():
        ...
        if selected_app.get() != "SELECT PROGRAM":
            ...
            Screentime_data = query.execute(
                "SELECT date, SUM(duration) FROM logs WHERE application =? GROUP BY date", (app_selected,))
...
def timerInstance(application, total_limit, total_time_elapsed):
    ...
    if time_elapsed > 0:
        query.execute("INSERT INTO logs (application, duration, date) VALUES (?, ?, ?)",
                      (application, time_elapsed, date_now))
        query.commit()
...
def timerStart(total_time_elapsed, application, snooze):
    ...
    label_guide = tk.Label(
        input_frame, text="Enter time limit (Hour : Minute : Seconds) ", font=("Arial", 10))
    ...
    if total_time_elapsed < total_limit or snooze is True:
        ...
        if instance_elapsed == total_limit or total_time_elapsed == total_limit:
            ...
            snooze = messagebox.askyesno(
                title="Snooze", message="Time limit reached.", detail="Would you like to snooze?", parent=root)
            mixer.music.stop()
        else:
            snooze = False
    else:
        ...
        mixer.music.play(loops=3)
        snooze = messagebox.askyesno(
            title="Snooze", message="Time limit is already reached.", detail="Would you like to snooze?", parent=root)
        mixer.music.stop()
...
def timerUpdate(total_time_elapsed, time_elapsed):
    ...
    # Update GUI text
    timer_text.config(text=str(hours_elapsed).zfill(2) + " : " +
                      str(minutes_elapsed).zfill(2) + " : " + str(second_elapsed).zfill(2))
    root.update()
...
def programSelection():
    ...
    programs_dropdown = ttk.Combobox(home_frame, state="readonly", textvariable=selected_program,
                                     postcommand=updateProgramlist, width=25, font=("Arial", 10))
    programs_dropdown["values"] = program_list
    programs_dropdown.current(0)
